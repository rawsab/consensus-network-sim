...
public class RandomEndToEndNetwork extends AbstractNetwork {
	...
	public RandomEndToEndNetwork(NodeSet ns, ClientSet cs, AbstractSampler s){
		super(ns, cs);
		this.sampler = s;
		CreateRandomNetwork(ns, cs);
	}
	...
	private void CreateRandomNetwork(NodeSet ns, ClientSet cs){
		for (int i=1; i <= ns.getNodes().size() + cs.getClientSetCount(); i++) {
			for (int j=1; j <= ns.getNodes().size() + cs.getClientSetCount(); j++) {
	            if(i!=j && Net[i][j] == 0)
	            {
	                Net[i][j] = (float) sampler.getNextConnectionThroughput(); //network throughput refers to how much data can be transferred from source to destination within a given time frame
	                Net[j][i] = Net[i][j];
